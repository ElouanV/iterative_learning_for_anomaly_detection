...
from models.deepsvdd.base.base_trainer import BaseTrainer
...
class DeepSVDDTrainer(BaseTrainer):
    ...
    def train(self, dataset: BaseADDataset, net: BaseNet, weighted=True):
        ...
        with tqdm(
            total=self.n_epochs,
            desc="Epoch",
            disable=not logger.isEnabledFor(logging.CRITICAL),
        ) as pbar:
            for epoch in range(self.n_epochs):
                ...
                for data in train_loader:
                    ...
                    dist = weights * dist if weighted else dist
                    if self.objective == "soft-boundary":
                        scores = dist - self.R**2
                        if weighted:
                            loss = self.R**2 + (1 / self.nu) * (
                        ...
                        if weighted:
                            loss = dist @ weights / torch.sum(weights)
                        else:
                ...
                pbar.update(1)
                pbar.set_postfix({"Loss: ": "{:.4f}".format(losses[-1])})
