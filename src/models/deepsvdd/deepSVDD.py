...
from models.deepsvdd.datasets.tabular import ADBenchDataset
from models.deepsvdd.networks.utils import build_autoencoder, build_network
...
class DeepSVDD(object):
    ...
    def train(
        ...
        # Get the model
        self.net, train_loss = self.trainer.train(
            dataset, self.net, weighted=weighted
        )
    ...
    def predict_score(self, X, device: str = "cuda"):
        ...
        with torch.no_grad():
            ...
            for inputs in data_loader:
                ...
                outputs = net(inputs)
                dist = torch.sum((outputs - self.trainer.c) ** 2, dim=1)
                if self.objective == "soft-boundary":
                    scores = dist - self.R**2
                else:
                    scores = dist
                scores_.append(scores)
            scores_ = torch.cat(scores_, dim=0).cpu().numpy()
        return scores_
    ...
    def fit(
        ...
        dataset = ADBenchDataset(X_train, y_train, weights)
        return self.train(
            dataset=dataset,
            optimizer_name="adam",
