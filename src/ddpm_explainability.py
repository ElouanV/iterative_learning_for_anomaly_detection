...
from models.ddpm.ddpm_explainer import DDPMExplainer
...
from viz.training_viz import (plot_anomaly_score_distribution,
                              plot_anomaly_score_distribution_split, plot_nmf, plot_tsne, reconstruction_error_boxplot)
...
def run_config(cfg, logger, device):
    ...
    ddpm_explainer = DDPMExplainer(model, device=device)

    anomaly_reconstruction_error, normal_reconstruction_error, w, h = (
        ddpm_explainer.feature_wise_explanation(
            X_eval=data["X_test"],
            y_eval=y_test_anomaly,
            saving_path=saving_path,
            n_components=4,
        )
    )
